...
public final class ClientPayloadHandlers {
    ...
    public static void handle(CustomPacketPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            ...
            } else if (payload instanceof TacticalHudStatePayload state) {
                TacticalHudClientPacketHandler.apply(state.enabled(), state.authorized());
            } else if (payload instanceof HudNightVisionStatePayload state) {
                TacticalHudClientPacketHandler.applyNightVision(state.enabled(), state.authorized());
            ...
            } else if (payload instanceof HudTrueInvisibilityStatePayload state) {
                TacticalHudClientPacketHandler.applyTrueInvisibility(state.enabled(), state.authorized());
            ...
            } else if (payload instanceof HudMagnetStatePayload state) {
                TacticalHudClientPacketHandler.applyMagnet(state.enabled(), state.authorized());
