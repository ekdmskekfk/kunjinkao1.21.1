...
| 覆写 HUD：FOV 写死 70、`partialTick` 未用、缺界面守卫 | ✅ **但首修有错**：坐标插值的辅助方法体被全局替换成自我调用 → 每渲染帧 StackOverflowError；全量重审抓到后已在 `e7ab5ac` 修回 | `c5d7cca` `e7ab5ac` |
| 伤害上限对生物失效（首修改错位置：只在 `LivingIncomingDamageEvent` 判断，而 `AttackEntityEvent` 已先取消原版攻击） | ✅ 改为在接管点判断：上限低于最大值就不接管 | `f43c078` `e7ab5ac` |
| `revoke` 只清背包槽（被撤销者可用潜影盒/丢地上保住全部能力） | ✅ 追加容器内容物与本人掉落的剑 | `ee1d029` `e7ab5ac` |
| 基岩/屏障开关要求"创造 + OP2"，与本模组授权模型矛盾 | ✅ 改为"密码管理员 或 原版游戏管理员" | `e7ab5ac` |
| 覆写清理无条件清备份（主手被换过时原件永久丢失） | ✅ 只在确实从备份恢复过时才清 | `e7ab5ac` |
| `-PautoBump` 下版本号可倒退、同名覆盖已有 jar | ✅ 默认构建直接用 `gradle.properties` 的版本，仅 `-PautoBump` 才 +1（实测产出 0.1.9 且不回写） | `e7ab5ac` |
| 剑设置界面仍有 5 处硬编码中文（3 个既有键成死键） | ✅ 全部改 translatable，补 3 键（各 110 键对齐） | `d07d08b` |
...
- **命令保护的覆盖代价**：触发从"任意命令"收紧到 `clear|item|data`（含命名空间写法）后，第三方模组/数据包自定义的清包命令不再触发快照，这类命令会真的删掉剑（旧实现同样覆盖不到函数内命令，但能覆盖第三方命令）。
- **加速方块的残余风险**：单方块每刻 4096 tick 预算已在，但加速器**数量无上限**；另外管理员曾授权时写进 NBT 的 1024 倍率，在撤销授权后仍按 NBT 原值运行（`loadAdditional` 不受 `mayEdit` 约束）。
- **HUD 回包被界面吞掉**（低）：按键后、回包前打开背包/箱子时，HUD 会置为开启但没有 HudScreen，功能栏看不到也点不到，需再按两次 H。
- **客户端乐观写入**（低）：未授权玩家按 K/I/P 或拖滑条时仍先改本地 NBT 再发包，服务端拒绝后本地显示与真实状态不符，直到重同步。
- **`/give`、创造模式、`/summon` 得到的剑不带 pending 标记**（见上文 §四.1，需你决定是否改成"绑定授权者 UUID"）。
...
- **一次全量对抗性重审**（针对 22 个提交的整条 diff，逐条对照 21.1.118 源码）：结论为"不成立"，即它**找到了真问题** —— 一是本文件此前记的"覆写 HUD 已修"实际引入了必崩的自我递归（已修），二是 P1-6 的伤害上限首修改错了位置（已修）；另外它确认授权链、pending 闸门、覆写落盘、客户端清理链、payload 收敛的 id 映射与注册数**均无绕过或错位**。这次复审的价值证明了"按批复核 + 一次全量复核"这套流程是必要的。
